export function getContracts({ network }: { network: string }) {
  ...
  xMetricJson = require(`core-evm-contracts/deployments/${network}/Xmetric.json`);
  questionAPIJson = require(`core-evm-contracts/deployments/${network}/QuestionAPI.json`);
  questionStateController = require(`core-evm-contracts/deployments/${network}/QuestionStateController.json`);
  bountyQuestionJson = require(`core-evm-contracts/deployments/${network}/BountyQuestion.json`);
  costController = require(`core-evm-contracts/deployments/${network}/ActionCostController.json`);
  vaultJson = require(`core-evm-contracts/deployments/${network}/Vault.json`);
