...
export function Score({ score }: Props) {
  ...
  return (
    <div
      className={clsx(
        SCORE_COLOR_SECONDARY[label] ? SCORE_COLOR_SECONDARY[label] : "",
        "rounded-lg h-9 flex text-sm w-fit"
      )}
    >
      <div
        className={clsx(
          SCORE_COLOR[label] ? SCORE_COLOR[label] : "",
          "h-full rounded-lg flex items-center justify-center w-20"
        )}
      >
        {label}
      </div>
