...
export default function AlertBanner ({setAlertContainerStatus, transactionStatus}: {setAlertContainerStatus: Dispatch<SetStateAction<boolean>>, transactionStatus: string}) {
    let text: string;
    ...
    return (
        <div className={`tw-px-4 tw-py-3 tw-rounded-lg tw-w-1/5 tw-fixed ${windowTop < 60 ? "tw-top-28" : "tw-top-6"} tw-left-10 tw-z-100 tw-bg-white alert`}>
            ...
            <Close20 onClick={() => setAlertContainerStatus(false)} className="tw-ml-auto tw-self-start tw-cursor-pointer" />
            </div>
        </div>
    )
}
